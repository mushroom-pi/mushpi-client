...
Production graph is **0** advisories and must stay 0 (`yarn audit:ci` = production + `--severity high`, exit 0); after the dev-toolchain refresh the **all-environment** count is 0 too (`yarn npm audit --recursive --no-deprecations` → "No audit suggestions"). Wired gates on `pre-push`: `audit:ci` and `knip:ci` run there (alongside `build` + `test`); `lint:ci` is the non-mutating zero-warnings ESLint gate form but is not in the hook chain — pre-commit's lint-staged runs `eslint --fix` without `--max-warnings=0`. The historical findings below were **dev-only** (tooling/install/build-time — never request-path, the client is a static SPA). Remediation followed the `yarn4-dependency-hygiene` skill: Stage A lockfile-only refresh first, then conservative direct-parent moves, pins last.
