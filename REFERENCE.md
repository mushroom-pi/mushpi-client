...
Production graph is **0** advisories and must stay 0 (`yarn audit:ci` = production + `--severity high`, exit 0). All recorded advisories here are **dev-only** (tooling/install/build-time — never request-path, the client is a static SPA). Remediation followed the `yarn4-dependency-hygiene` skill: Stage A lockfile-only refresh first, then conservative direct-parent moves, pins last.
