...
Topics covered here: path aliases · image handling · `useAsyncWithToast` · polling gotchas · Recharts chart conventions · directory & file detail · client regeneration · testing (Vitest) detail · component & mutation gotchas · known quirks · feature palettes · Knip · dependency hygiene (audit remediation & pins) · Husky hooks & exec-bit fragility.
...
**Generated-directory attribution (gitignored ⇒ git-visible-free churn).** `src/api/generated/**` is gitignored, so its content can change with **no git-visible event** — any `yarn gen:*` run rewrites it silently and `git status` stays clean. A build failure caused by regenerated generated code therefore **survives `git stash` / `git checkout` unchanged**: attributing it by `git status` or by reverting the lockfile proves nothing. Attribute by comparing generated-file mtimes (`ls -l --time-style=full-iso src/api/generated/`) against commit times (`git log -1 --format=%cI`). Corollary: `tsc -b`'s incremental state lives in `node_modules/.tmp/tsconfig.{app,node}.tsbuildinfo` (untracked, per `tsBuildInfoFile` in both tsconfigs) — so `npx tsc -b --force` is the authoritative cold check; a warm `yarn build` can mislead when generated content shifted under a stale tsbuildinfo.

**Emitted bytes depend on the generator JAR, not the npm wrapper.** The output is a function of the JAR version pinned in `openapitools.json` (`generator-cli.version`, currently 7.14.0) — the `@openapitools/openapi-generator-cli` npm **wrapper** version has zero effect on emitted bytes; it only downloads and launches the JAR. Wrapper upgrades are therefore low-risk; for generation-output review the version that matters is the JAR pin. Operational caveat: the downloaded JAR cache lives in `node_modules/@openapitools/openapi-generator-cli/versions/` and is **destroyed by any `yarn up` of the wrapper** — the next `gen:client` then re-downloads ~30 MB, so a network-less environment must re-seed the cache before regenerating.
...
- **Gate scoping — deliberately differs from the server**: `knip:ci` = `knip --include files,dependencies,unlisted --exclude devDependencies --no-gitignore`. The server's `--dependencies` form is **blind to unused files** — the issue class that actually fires in this repo (dead files) — so the frontend gate adds `files`. In knip 5, `--include dependencies` also surfaces devDependencies findings (historical: the bare `--include files,dependencies` form printed 10 of them), hence the `--exclude devDependencies`; those 10 devDependencies have since been **removed**, and the full `yarn knip` scan now prints no unused-devDependencies section at all. The exclusion is kept as deliberate gate scoping (the gate watches the runtime graph), not an outstanding cleanup. `unlisted` (imported-but-undeclared) is in scope: zero findings today, so it catches future regressions without pre-existing red. exports/types/duplicates are deliberately out of scope (~180 findings, mostly permanent generated-client noise).
...
## Dependency Hygiene (Audit Remediation & Pins)

Production graph is **0** advisories and must stay 0 (`yarn audit:ci` = production + `--severity high`, exit 0); after the dev-toolchain refresh the **all-environment** count is 0 too (`yarn npm audit --recursive --no-deprecations` → "No audit suggestions"). Wired gates on `pre-push`: `audit:ci`, `lint:ci` and `knip:ci` run there (alongside `build` + `test`); `lint:ci` is the non-mutating zero-warnings ESLint gate form (rationale: §Husky) — pre-commit's lint-staged runs `eslint --fix` without `--max-warnings=0`, so warnings can pass pre-commit. The historical findings below were **dev-only** (tooling/install/build-time — never request-path, the client is a static SPA). Remediation followed the `yarn4-dependency-hygiene` skill: Stage A lockfile-only refresh first, then conservative direct-parent moves, pins last.

- **Stage A — `yarn up -R` refresh cleared 65 of 67 advisories with no `package.json` change** (`git diff --exit-code -- package.json` stayed 0). Every transitive (and `vite` itself) had a patched version *inside* its declared parent range: vite `^7.1.7`→7.3.6 (a direct dep but in-range minor — the range string is unchanged, only the lockfile resolution moved), then rollup/postcss/nanoid/picomatch/browserslist/baseline-browser-mapping/@babel/core, the eslint chain (js-yaml/minimatch/brace-expansion/flatted/@humanfs/node/ajv@6), ajv@8→fast-uri, and node-gyp→tar/glob. **Convention: always try `yarn up -R <transitive>` before any pin** — most advisories are a stale lockfile, not a stale range.
- **Stage B — `vitest` + `@vitest/coverage-v8` `^3.2.4` → `^4.1.11` (devDependency major).** The Vitest path-traversal advisory (GHSA-82fw-gwwq-j7x9) is patched **only at 4.1.11**, outside the `^3.2.4` (<4.0.0) cap — no in-range fix existed. `@vitest/mocker` is **hard-pinned exact `npm:3.2.7` by vitest 3.x** and ships the same advisory, so it cannot be moved independently; the vitest@4 move pulls it to patched 4.1.11 as a side effect. Vitest 4 is a clean drop-in for this repo (same `vitest.config.ts` options, all 71 tests pass, peers satisfied: vite `^6.4||^7||^8`, `@types/node >=24`, jsdom `*`). **Do not add a separate `resolutions` pin for `@vitest/mocker`** — it must stay version-identical to its vitest parent (a mismatched mocker major is unsupported).
- **`resolutions` pin — `"node-gyp": "npm:11.4.2"`.** `fsevents` (a macOS-only, **optional** dependency of vite/rollup, `conditions: os=darwin`) declares `node-gyp: "npm:latest"` — a *floating* tag that can silently move node-gyp majors on any fresh/cache-missing install, reshaping the `node-gyp → tar` and `node-gyp → make-fetch-happen → cacache → glob` graphs without a manifest change. This pin freezes the exact `11.4.2` graph the ladder validated at 0 advisories; it is **drift prevention, not an advisory fix** (node-gyp's `tar@7.5.1`/`glob@10.4.5` were already cleared in-range by Stage A). node-gyp 13.x exists but is deliberately **not** adopted — no advisory demands it and adopting it would churn the validated green graph. **Removal condition:** drop the pin once `fsevents` stops declaring `node-gyp: npm:latest` (still floats as of fsevents 2.3.3) or once the toolchain no longer pulls fsevents; then let node-gyp float again. This bullet is the pin's rationale + removal condition's **only home** — `package.json` is strict JSON and cannot hold a comment.
- **Standing acceptance (if a recurrence appears):** the `fsevents → node-gyp → tar/glob` chain is darwin-only, optional, and install/native-build-time — never request-path. Per the skill, a build/install-time-only finding there may be **accepted with rationale** rather than force-upped; prefer moving the parent chain (as vitest@4 did) over forcing a child major under a parent whose range excludes it.

## Husky Exec-Bit Fragility

**Hook lineup (v9)**: three hooks, one command each, dispatched by the generated `.husky/_/h` wrapper. Wiring: `"prepare": "husky"` script + `core.hooksPath=.husky/_`. `pre-commit` = `yarn lint-staged` — staged-scoped `eslint --fix` + `prettier --write` (deliberately not the skill's whole-repo `format` + `lint` pattern; staged-scoping is this repo's established behaviour). `commit-msg` = `yarn commitlint --edit "$1"` — `$1` is the commit-message file; merge/revert/fixup/squash messages are skipped by source-based `ignores`, not a hook guard (see next). `pre-push` = `yarn audit:ci && yarn lint:ci && yarn build && yarn knip:ci && yarn test` (cheapest static checks first; `build` (`tsc -b`) stays before the transpile-only `test` suite so type errors surface first; there is no `test:e2e` here). Rationale:
...
- **`lint:ci` in the pre-push chain** — the only thing enforcing zero warnings repo-wide: there is no client CI workflow yet, and pre-commit's lint-staged `eslint --fix` (no `--max-warnings=0`) permits warnings through.
